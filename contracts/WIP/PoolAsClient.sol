...
contract Pool {
    ...
    function getWinningRounds(struct finalData) public {
        // we get this from the sportsdata.io api
        uint256[] _roundOneWinners;
        uint256[] _roundTwoWinners;
        uint256[] _roundThreeWinners;
        uint256[] _roundFourWinners;
        uint256[] _roundFiveWinners;
        uint256 _overallWinner;

        for (uint256 i = 0; i < finalData.games.length; i++) {
            struct currentRound = finalData.games[i];
            if ( currentRound.round == 11) {
                if (currentRound.AwayTeamScore > currentRound.homeTeamScore) {
                    _overallWinner = currentRound.awayTeam;
                } else {
                    _overallWinner = currentRound.homeTeam;
                }
                _roundFiveWinners = [currentRound.awayTeam, currentRound.homeTeam];
            } else if (currentRound.round == 9) {
                _roundFourWinners.push(currentRound.awayTeam);
                _roundFourWinners.push(currentRound.homeTeam);
            } else if (currentRound.round == 7) {
                _roundThreeWinners.push(currentRound.awayTeam);
                _roundThreeWinners.push(currentRound.homeTeam);
            } else if (currentRound.round == 5) {
                _roundTwoWinners.push(currentRound.awayTeam);
                _roundTwoWinners.push(currentRound.homeTeam);
            } else if (currentRound.round == 4) {
                _roundOneWinners.push(currentRound.awayTeam);
                _roundOneWinners.push(currentRound.homeTeam);
            }
